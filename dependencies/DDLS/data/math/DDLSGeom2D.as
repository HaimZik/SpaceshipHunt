package DDLS.data.math
{
	...
	public class DDLSGeom2D
	{
		...
		static public function isCircleIntersectingAnyConstraint(x:Number, y:Number, radius:Number, mesh:DDLSMesh):Boolean
		{
			...
			{
				if ((loc as DDLSVertex).edge)
				{
				face = (loc as DDLSVertex).edge.leftFace;
				}else
				{
					trace("in isCircleIntersectingAnyConstraint edge was null");
					return true;
				}
			}
			...
			edgesToCheckHelper.length=3;
			edgesToCheckHelper[0]=face.edge;
			edgesToCheckHelper[1]=face.edge.nextLeftEdge;
			edgesToCheckHelper[2]=face.edge.nextLeftEdge.nextLeftEdge;
		...
		static public function getDirection(x1:Number, y1:Number, x2:Number, y2:Number, x3:Number, y3:Number):int
		{
		...
		// 1 if the path goes to the left
		// -1 if the path goes to the right
		static public function getDirection2(x1:Number, y1:Number, x2:Number, y2:Number, x3:Number, y3:Number):int
		{
			// dot product with the orthogonal vector pointing left vector of eUp:
			var dot:Number = (x3 - x1) * (y2 - y1) + (y3 - y1) * (-x2 + x1);
		...
		static public function clipSegmentByTriangle(s1x:Number, s1y:Number, s2x:Number, s2y:Number, t1x:Number, t1y:Number, t2x:Number, t2y:Number, t3x:Number, t3y:Number, pResult1:DDLSPoint2D = null, pResult2:DDLSPoint2D = null):Boolean
		{
			var side1_1:int;
			var side1_2:int;
		...
		static public function isSegmentIntersectingTriangle(s1x:Number, s1y:Number, s2x:Number, s2y:Number, t1x:Number, t1y:Number, t2x:Number, t2y:Number, t3x:Number, t3y:Number):Boolean
		{
			// check sides
		...
		static public function intersections2edges(edge1:DDLSEdge, edge2:DDLSEdge, posIntersection:DDLSPoint2D = null, paramIntersection:Vector.<Number> = null, infiniteLineMode:Boolean = false):Boolean
		{
		...
		static public function intersections2Circles(cx1:Number, cy1:Number, r1:Number, cx2:Number, cy2:Number, r2:Number, result:Vector.<Number> = null):Boolean
		{
			...
			{
				var transcendPart:Number = Math.sqrt(((r1 + r2) * (r1 + r2) - distRadiusSQRD) * (distRadiusSQRD - (r2 - r1) * (r2 - r1)));
		...
		public static function intersectionsSegmentCircle(p0x:Number, p0y:Number, p1x:Number, p1y:Number, cx:Number, cy:Number, r:Number, result:Vector.<Number> = null):Boolean
		{
		...
		public static function intersectionsLineCircle(p0x:Number, p0y:Number, p1x:Number, p1y:Number, cx:Number, cy:Number, r:Number, result:Vector.<Number>):Boolean
		{
			var p0xSQD:Number = p0x * p0x;
			var p0ySQD:Number = p0y * p0y;
			var a:Number = p1y * p1y - 2 * p1y * p0y + p0ySQD + p1x * p1x - 2 * p1x * p0x + p0xSQD;
