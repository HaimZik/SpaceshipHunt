...
package starling.core
{
	...
	public class Starling extends EventDispatcher
	{
		...
		// members
		private const EARLY_CLEAR:Boolean = true;
		...
		//	}
			...
			if (doRedraw)
			{
				...
				_painter.state.setProjectionMatrix(_viewPort.x < 0 ? -_viewPort.x / scaleX : 0.0, _viewPort.y < 0 ? -_viewPort.y / scaleY : 0.0, _clippedViewPort.width / scaleX, _clippedViewPort.height / scaleY, _stage.stageWidth, _stage.stageHeight, _stage.cameraPosition);
				
				if (!shareContext && !EARLY_CLEAR)
					_painter.clear(stageColor, Color.getAlpha(stageColor));
		...
		private function updateViewPort(forceUpdate:Boolean = false):void
		{
			...
			if (forceUpdate || !RectangleUtil.compare(_viewPort, _previousViewPort))
			{
				...
				var contentScaleFactor:Number = _supportHighResolutions ? _nativeStage.contentsScaleFactor : 1.0;
				
				if (_supportBrowserZoom)
					contentScaleFactor *= _nativeStage.browserZoomFactor;
				
				_painter.configureBackBuffer(_clippedViewPort, contentScaleFactor, _antiAliasing, true, _supportBrowserZoom);
		...
		{
			var stageBounds:Rectangle = Pool.getRectangle(0, 0, _nativeStage.stageWidth, _nativeStage.stageHeight);
		...
		private function onTouch(event:Event):void
		{
			...
			else
			{
				...
				if (_supportsCursor && touchEvent.isPrimaryTouchPoint)
					return;
				else
				{
		...
		private function hitTestNativeOverlay(localX:Number, localY:Number):Boolean
		{
			if (_nativeOverlay.numChildren)
			{
				...
				var matrix:Matrix = Pool.getMatrix(_nativeOverlay.scaleX, 0, 0, _nativeOverlay.scaleY, _nativeOverlay.x, _nativeOverlay.y);
		...
		private function updateStatsDisplayPosition():void
		{
			...
			var scaleX:Number = _viewPort.width / _stage.stageWidth;
			var scaleY:Number = _viewPort.height / _stage.stageHeight;
			var clipping:Rectangle = Pool.getRectangle(_viewPort.x < 0 ? -_viewPort.x / scaleX : 0.0, _viewPort.y < 0 ? -_viewPort.y / scaleY : 0.0, _clippedViewPort.width / scaleX, _clippedViewPort.height / scaleY);
			
			if (horizontalAlign == Align.LEFT)
				_statsDisplay.x = clipping.x;
			else if (horizontalAlign == Align.RIGHT)
				_statsDisplay.x = clipping.right - _statsDisplay.width;
			else if (horizontalAlign == Align.CENTER)
				_statsDisplay.x = (clipping.right - _statsDisplay.width) / 2;
			else
				throw new ArgumentError("Invalid horizontal alignment: " + horizontalAlign);
			
			if (verticalAlign == Align.TOP)
				_statsDisplay.y = clipping.y;
			else if (verticalAlign == Align.BOTTOM)
				_statsDisplay.y = clipping.bottom - _statsDisplay.height;
			else if (verticalAlign == Align.CENTER)
				_statsDisplay.y = (clipping.bottom - _statsDisplay.height) / 2;
			else
		...
		{
			if (!value)
				_previousViewPort.setEmpty(); // forces back buffer update
		...
		public function set supportBrowserZoom(value:Boolean):void
		{
			if (_supportBrowserZoom != value)
			{
				...
				if (value)
					_nativeStage.addEventListener(Event.BROWSER_ZOOM_CHANGE, onBrowserZoomChange, false, 0, true);
				else
					_nativeStage.removeEventListener(Event.BROWSER_ZOOM_CHANGE, onBrowserZoomChange, false);
			}
		}
