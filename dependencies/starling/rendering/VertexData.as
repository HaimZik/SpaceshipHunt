...
package starling.rendering
{
	...
	public class VertexData
	{
		...
		private function copyAttributeTo_internal(target:VertexData, targetVertexID:int, matrix:Matrix, sourceAttribute:VertexDataAttribute, targetAttribute:VertexDataAttribute, vertexID:int, numVertices:int):void
		{
			...
			var matrixA:Number = matrix.a;
			var matrixB:Number = matrix.b;
			var matrixC:Number = matrix.c;
			var matrixD:Number = matrix.d;
			var matrixTx:Number = matrix.tx;
			var matrixTy:Number = matrix.ty;
			if (matrix)
			{
				if (starling_internal::tryAssignToDomainMemory(targetData, (targetDelta + 8) * numVertices))
				{
					if (matrixA == 1 && matrixC == 0 && matrixB == 0 && matrixD == 1 && matrixTx==0 && matrixTy==0)
					{
						for (i = 0; i < numVertices; ++i)
						{
							// copy into targetData.
							si8(sourceData[sourcePos], pos);
							si8(sourceData[++sourcePos], pos + 1);
							si8(sourceData[++sourcePos], pos + 2);
							si8(sourceData[++sourcePos], pos + 3);
							si8(sourceData[++sourcePos], pos + 4);
							si8(sourceData[++sourcePos], pos + 5);
							si8(sourceData[++sourcePos], pos + 6);
							si8(sourceData[++sourcePos], pos + 7);
							pos += 8 + targetDelta;
		...
		public function createVertexBuffer(upload:Boolean = false, bufferUsage:String = "staticDraw"):VertexBuffer3D
		{
			...
			var buffer:VertexBuffer3D = context.createVertexBuffer(_numVertices, _vertexSize / 4, bufferUsage);
