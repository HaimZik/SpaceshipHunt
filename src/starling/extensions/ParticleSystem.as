...
package starling.extensions
{
	...
	public class ParticleSystem extends Mesh implements IAnimatable
	{
		...
		private function updateBlendMode():void
		{
			...
			if (_blendFactorSource == Context3DBlendFactor.ONE && _blendFactorDestination == Context3DBlendFactor.ONE_MINUS_SOURCE_ALPHA)
			{
		...
		public function advanceTime(passedTime:Number):void
		{
			...
			if (_emissionTime > 0)
			{
				...
				if (_numParticles < maxNumParticles && _numParticles + (_frameTime / timeBetweenParticles) > maxNumParticles)
				{
			...
			{
				var vertexSize:int = _vertexData.vertexSize;
				for (var p:int = 0; p < _numParticles; ++p)
				{
					...
					var position:int = vertexID * vertexSize;
					if (rotation)
					{
						...
						sf32(x - cosX + sinY, position);
						sf32(y - sinX - cosY, position + 4);
						position += vertexSize;
						sf32(x + cosX + sinY, position);
						sf32(y + sinX - cosY, position + 4);
						position += vertexSize;
					...
					else
					{
						...
						sf32(x - offsetX, position);
						sf32(y - offsetY, position + 4);
						position += vertexSize;
						sf32(x + offsetX, position);
						sf32(y - offsetY, position + 4);
						position += vertexSize;
		...
		public function get emitterNextX():Number
		{
			return _emitterNextX;
		}
		
		public function set emitterNextX(value:Number):void
		{
			_emitterNextX = value;
		}
		...
		public function get emitterNextY():Number
		{
			return _emitterNextY;
		}
		
		public function set emitterNextY(value:Number):void
		{
			_emitterNextY = value;
		}
